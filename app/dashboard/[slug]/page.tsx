            ...
            <dragcontext.Provider value={{ dragtask, setdragtask }}>
                <context.Provider value={{ Tasks, setTasks }}>
                    <SkeletonTheme baseColor="#202020" highlightColor="#444">
                        <div className="text-white flex justify-center items-center gap-6 mt-10 w-full max-sm:gap-3">
                            {session?.user?.image ? <> <img className="rounded-full w-14" src={`${session?.user?.image}`} alt="userimage" />
                                <div className="flex flex-col gap-1 max-sm:gap-0">
                                    <div className="text-2xl leading-none font-bold max-sm:text-lg">Welcome {session?.user?.name}</div>
                                    <div className="text-lg leading-none text-neutral-300 font-semibold max-sm:text-base">{session?.user?.email}</div>
                                </div>
                                ...
                                : <div className="flex gap-5 max-sm:px-3 max-sm:mt-5 max-sm:mb-10">
                                    <Skeleton circle={true} height={60} width={60} />
                                    <Skeleton height={55} width={320} />
                                </div>}
                        </div>
                        <div className="flex text-white min-h-screen gap-5 p-32 h-[92vh] w-full justify-center items-center max-sm:flex-col max-sm:px-10">
                            ...
                            <IconContext.Provider value={{ color: "#6e6e6e", size: "2em" }}>
                                {deleteelement === false && <div onDragEnter={D_enter} className="w-[25%] h-72 border self-start bg-[#262626] border-neutral-600 flex justify-center items-center rounded-lg max-sm:w-full max-sm:h-96 max-sm:self-auto">
                                    <FiTrash />
                                </div>}
                            </IconContext.Provider>
                            <IconContext.Provider value={{ color: "red", size: "2em" }}>
                                {deleteelement === true && <div onDragLeave={D_Leave} className="w-[25%] h-72 border self-start bg-red-950 border-red-600 flex justify-center items-center rounded-lg max-sm:w-full">
                                    <motion.div
                                        initial={{ y: 0 }}
                                        animate={{ y: [0, 5, -5, 0] }}
